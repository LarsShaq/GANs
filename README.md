...
There has been some GAN architectures for transforming one image into another one, but specialized for certain applications (like segmentation-map to image). Here the authors provide a general framework for translating one image into another. For this they use two losses for the GAN:
